...
function get_the_category_rss($type = null) {
	...
	foreach ( $tag_names as $tag_name ) {
		...
		elseif ( 'atom' == $type )
			$the_list .= sprintf( '<tag scheme="%1$s" term="%2$s" />', esc_attr( apply_filters( 'get_bloginfo_rss', get_bloginfo( 'url' ) ) ), esc_attr( $tag_name ) );
		else
			$the_list .= "\t\t<tag><![CDATA[" . @html_entity_decode( $tag_name, ENT_COMPAT, get_option('blog_charset') ) . "]]></tag>\n";
	}
